...
function BiddingProductScreen() {
  ...
  const {
    loading: loadingProductBid,
    error: errorProductBid,
    success: successProductBid,
  } = productBidCreate;
  ...
  const canBid =
    endTime > current ||
    (endTime.getDate() === current.getDate() &&
      endTime.getMonth() === current.getMonth() &&
      endTime.getFullYear() === current.getFullYear());
  ...
  const submitBidHandler = (e) => {
    ...
    dispatch(
      createProductBid(id, { id, amount: parsedAmount, highest_bidder })
    );
  };
  ...
  return (
    <div>
      <Link to="/biddings" className="btn btn-light go-back-btn">
        ...
        <div>
          <Row>
            <Col md={6} className="w-50">
              <Image
                src={`${SEED}${bidding_product.image}`}
                alt={bidding_product.name}
                fluid
                height="300px"
                width="400px"
              />
                      ...
                      <Row>
                        {userInfo && canBid ? (
                          ...
                          <p>
                            Please{" "}
                            {userInfo ? (
                              "wait for the next bidding event"
                            ) : (
                              <Message variant="info">
                                Please <Link to="/login">login</Link> to bid.
                              </Message>
                            )}
                            .
                          </p>
          ...
          <Row>
            <Col md={6}>
              <h4 className="auto-underline-animation">Bids</h4>
              {bids.length > 0 ? (
